 ...
 * @version   $Id: geshi.php 3170 2009-04-18 14:48:34Z catzwolf $
 *
 */
...
class GeSHi
{
    ...
    {
        if (GESHI_NO_LINE_NUMBERS != $flag && GESHI_NORMAL_LINE_NUMBERS != $flag && GESHI_FANCY_LINE_NUMBERS != $flag) {
    ...
    {
        $this->language_data['KEYWORDS'][$key] = array_diff($this->language_data['KEYWORDS'][$key], array(
            $word));
    }
    ...
    function parse_code()
    {
        ...
        foreach($parts as $key => $data) {
            ...
            if ($key % 2) {
                ...
                if (! $this->strict_mode || $this->language_data['HIGHLIGHT_STRICT_BLOCK'][$script_key]) {
                    ...
                    for($i = 0; $i < $length; $i ++) {
                        ...
                        if (($this->line_numbers != GESHI_NO_LINE_NUMBERS || count($this->highlight_extra_lines) > 0) && $char == "\n") {
                            ...
                            $stuff_to_parse = '';
                        } elseif ($hq && substr($part, $i, strlen($hq)) == $hq && ($STRING_OPEN == '') && $this->lexic_permissions['STRINGS']) {
                ...
                if ($this->strict_mode && $this->language_data['STYLES']['SCRIPT'][$script_key] != '' && $this->lexic_permissions['SCRIPT']) {
                    $result .= '</span>';
                }
            } else {
        ...
        if (isset($this->language_data['URLS'][$group]) && $this->language_data['URLS'][$group] != '' && substr($keyword, 0, 5) != '&lt;/') {
            ...
            if ($start_or_end == 'BEGIN') {
                ...
                if ($keyword != '') {
                    ...
                    return '<|UR1|"' . str_replace(array(
                        '{FNAME}' , 
                        '.'), array(
                        @htmlspecialchars($word, ENT_COMPAT, $this->encoding) , 
                        '<DOT>'), $this->language_data['URLS'][$group]) . '">';
                }
                return '';
    ...
    function parse_non_string_part(&$stuff_to_parse)
    {
        ...
        foreach($this->language_data['REGEXPS'] as $key => $regexp) {
            if ($this->lexic_permissions['REGEXPS'][$key]) {
                if (is_array($regexp)) {
                    $stuff_to_parse = preg_replace("/" . str_replace('/', '\/', $regexp[GESHI_SEARCH]) . "/{$regexp[GESHI_MODIFIERS]}", "{$regexp[GESHI_BEFORE]}<|!REG3XP$key!>{$regexp[GESHI_REPLACE]}|>{$regexp[GESHI_AFTER]}", $stuff_to_parse);
                } else {
                    ...
                    foreach($keywordset as $keyword) {
                        ...
                        if (false !== stristr($stuff_to_parse_pregquote, $keyword)) {
                            ...
                            if ($this->language_data['CASE_SENSITIVE'][$k]) {
                                $stuff_to_parse = preg_replace("/([^a-zA-Z0-9\$_\|\#;>|^])($keyword)(?=[^a-zA-Z0-9_<\|%\-&])/e", "'\\1' . $func2('\\2', '$k', 'BEGIN') . '<|$styles>' . $func('\\2') . '|>' . $func2('\\2', '$k', 'END')", $stuff_to_parse);
                            } else {
                                // Change the case of the word.
                                // hackage again... must... release... 1.2...
                                if ('smarty' == $this->language) {
                                    $hackage = '\/';
                                } else {
                                    $hackage = '';
                                }
                                $stuff_to_parse = preg_replace("/([^a-zA-Z0-9\$_\|\#;>$hackage|^])($keyword)(?=[^a-zA-Z0-9_<\|%\-&])/ie", "'\\1' . $func2('\\2', '$k', 'BEGIN') . '<|$styles>' . $func('\\2') . '|>' . $func2('\\2', '$k', 'END')", $stuff_to_parse);
                            }
        ...
        foreach($this->language_data['REGEXPS'] as $key => $regexp) {
            if ($this->lexic_permissions['REGEXPS'][$key]) {
                if (! $this->use_classes) {
                    ...
                    if (is_array($this->language_data['REGEXPS'][$key]) && array_key_exists(GESHI_CLASS, $this->language_data['REGEXPS'][$key])) {
                        $attributes = ' class="' . $this->language_data['REGEXPS'][$key][GESHI_CLASS] . '"';
                    } else {
    ...
    function finalise($parsed_code)
    {
        ...
        if ($this->enable_important_blocks && (strstr($parsed_code, @htmlspecialchars(GESHI_START_IMPORTANT, ENT_COMPAT, $this->encoding)) === false)) {
    ...
    function footer()
    {
        ...
        if (GESHI_HEADER_NONE == $this->header_type) {
            return ($this->line_numbers != GESHI_NO_LINE_NUMBERS) ? '</ol>' . $footer_content : $footer_content;
        }
    ...
    function get_stylesheet($economy_mode = true)
    {
        ...
        foreach($this->language_data['STYLES']['REGEXPS'] as $group => $styles) {
            if (! $economy_mode || ! ($economy_mode && $styles == '') && ! ($economy_mode && ! $this->lexic_permissions['REGEXPS'][$group])) {
                if (is_array($this->language_data['REGEXPS'][$group]) && array_key_exists(GESHI_CLASS, $this->language_data['REGEXPS'][$group])) {
                    $stylesheet .= "$selector.";
