 ...
 * @version $Id: dbupdater.php 331 2007-12-23 16:01:11Z malanciault $
...
class SmartsectionTable {
    ...
    *
    */		
	function SmartsectionTable($name) 
	{
		$this->_name = $name;
		$this->_data = array();
	}
    ...
    */		
	function addData()
	{
	    ...
	    foreach ($this->getData() as $data) {
		    ...
		    if (! $ret) {
	        	echo "<li class='err'>" . sprintf(_AM_SSECTION_DB_MSG_ADD_DATA_ERR, $this->name()) . "</li>";
	    	} else {
		        echo "<li class='ok'>" . sprintf(_AM_SSECTION_DB_MSG_ADD_DATA, $this->name()) . "</li>";
		    }      	
	    }
    ...
    */		
	function createTable()
	{
	    ...
	    $ret = $xoopsDB->query($query);
	    if (! $ret) {
	        echo "<li class='err'>" . sprintf(_AM_SSECTION_DB_MSG_CREATE_TABLE_ERR, $this->name()) . "</li>";
	    } else {
	        echo "<li class='ok'>" . sprintf(_AM_SSECTION_DB_MSG_CREATE_TABLE, $this->name()) . "</li>";
	    }   
    ...
    */		
	function dropTable()
	{
	    ...
	    $ret = $xoopsDB->query($query);
	    if (! $ret) {
	        echo "<li class='err'>" . sprintf(_AM_SSECTION_DB_MSG_DROP_TABLE_ERR,  $this->name()) . "</li>";
	        return false;
	    } else {
	        echo "<li class='ok'>" . sprintf(_AM_SSECTION_DB_MSG_DROP_TABLE,  $this->name()) . "</li>";
	        return true;
	    }   
	}	
    ...
    */		
	function alterTable()
	{
	    ...
	    foreach ($this->getAlteredFields() as $alteredField) {
		    ...
		    if (! $ret) {
		        echo "<li class='err'>" . sprintf(_AM_SSECTION_DB_MSG_CHGFIELD_ERR, $alteredField['name'], $this->name()) . "</li>";
		    } else {
		        echo "<li class='ok'>" . sprintf(_AM_SSECTION_DB_MSG_CHGFIELD,$alteredField['name'], $this->name()) . "</li>";
		    }   
	    }
    ...
    */		
	function addNewFields()
	{
	    ...
	    foreach ($this->getNewFields() as $newField) {
		    ...
		    $ret = $ret && $xoopsDB->query($query);
		    if (! $ret) {
		        echo "<li class='err'>" . sprintf(_AM_SSECTION_DB_MSG_NEWFIELD_ERR, $newField['name'], $this->name()) . "</li>";
		    } else {
		        echo "<li class='ok'>" . sprintf(_AM_SSECTION_DB_MSG_NEWFIELD,$newField['name'], $this->name()) . "</li>";
		    }   
	    }
    ...
    */		
	function updateFieldsValues()
	{
	    ...
	    foreach ($this->getUpdatedFields() as $updatedField) {
		    ...
		    $ret = $ret && $xoopsDB->query($query);
		    if (! $ret) {
		        echo "<li class='err'>" . sprintf(_AM_SSECTION_DB_MSG_UPDATE_TABLE_ERR, $this->name()) . "</li>";
		    } else {
		        echo "<li class='ok'>" . sprintf(_AM_SSECTION_DB_MSG_UPDATE_TABLE, $this->name()) . "</li>";
		    }   
	 	}
    ...
    */		
	function dropFields()
	{
	    ...
	    foreach ($this->getdropedFields() as $dropedField) {
		    ...
		    $ret = $ret && $xoopsDB->query($query);
		    if (! $ret) {
		        echo "<li class='err'>" . sprintf(_AM_SSECTION_DB_MSG_DROPFIELD_ERR, $dropedField, $this->name()) . "</li>";
		    } else {
		        echo "<li class='ok'>" . sprintf(_AM_SSECTION_DB_MSG_DROPFIELD, $dropedField, $this->name()) . "</li>";
		    }   
	    }
	    return $ret;
	}		
...
class SmartsectionDbupdater {
    
	function SmartsectionDbupdater() 
	{
    ...
    */		
	function renameTable($from, $to)
	{
	    ...
	    $ret = $xoopsDB->query($query);
	    if (! $ret) {
	        echo "<li class='err'>" . sprintf(_AM_SSECTION_DB_MSG_RENAME_TABLE_ERR, $from) . "</li>";
	        return false;
	    } else {
	        echo "<li class='ok'>" . sprintf(_AM_SSECTION_DB_MSG_RENAME_TABLE, $from, $to) . "</li>";
	        return true;
	    }   
	}	
    ...
    *
    * @param object $table {@link SmartsectionTable} that will be updated
    *
    * @see SmartsectionTable
    *
    * @return bool true if success, false if an error occured
    */		
