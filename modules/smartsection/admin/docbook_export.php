...
if ($op == 'go')
{
    Header("Content-Disposition: attachment; filename=smartsection.xml");
    Header("Connection: close");
    Header("Content-Type: text/xml; name=smartsection.xml");
    ...
    $resultC = $xoopsDB->query ("select * from ".$xoopsDB->prefix("smartsection_categories"));
    while ($arrC = $xoopsDB->fetchArray ($resultC))
    {
        ...
        $resultQ = $xoopsDB->query ("select * from ".$xoopsDB->prefix("smartsection_item")." where categoryid=$c_categoryid");
