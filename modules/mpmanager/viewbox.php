...
//'msg_text' => $myts->displayTarea($myts->undoHtmlSpecialChars($pm[$i]->getVar('msg_text')),$xoopsModuleConfig['html']),

$hidden_quote .= "\n".$myts->xoopsCodeDecode($pm[$i]->getVar('msg_text'))."[/quote]";
...
//$mp_treesubject = $pm_tree[$i]->getVar('subject') ? $myts->displayTarea($pm_tree[$i]->getVar('subject')) : _MP_NONESUBJ ;
 ...
 "post_image" => $tree_img,  
 "post_title" => '<a href="viewbox.php?op=view&start='.$start.'&sortname='.$sortname.'&sortorder='.$sortorder.'&vieworder='.$vieworder.'&treesortorder='.$treesortorder.'&catbox='.$catbox.'&viewstart='.$viewstart.'&msg_thread='.$pm_tree[$i]->getVar('msg_id').'#'.$pm_tree[$i]->getVar('msg_id').'">'.$mp_treesubject.'</a>', 
 "post_prefix" => $prefix, 
 "poster" => $poster));
...
$box_actions[] = "<input type='submit' class='xo-message-form'  onclick='document.prvmsg.action=\"msgbox.php?op=sendbox&reply=1\"' id='reply' value='"._MP_MREPLY."'>";
} }else {
...
$box_actions[] = "<input type='submit' class='xo-message-form'  onclick='document.prvmsg.action=\"delbox.php?option=delete_messagess\"' id='del'  value='"._MP_MDEL."'>";
$box_actions[] = "<input type='submit' class='xo-message-form'  onclick='document.prvmsg.action=\"delbox.php?option=read_messagess&read=0\"' id='nlu'  value='"._MP_MNLU."'>";
$box_actions[] = "<input type='submit' class='xo-message-form'  onclick='document.prvmsg.action=\"delbox.php?option=move_messagess\"' id='move'  value='"._MP_MMOVE."'>";
