...
function formulaire_menu_show() {
  ...
  $gperm_handler = &xoops_gethandler( 'groupperm' );
	...
	$sql = "SELECT f.id_form, f.desc_form, f.help , m.indent, m.margintop, m.marginbottom, m.itemurl, m.bold FROM " . $xoopsDB->prefix("form_id") . " f LEFT JOIN " . $xoopsDB->prefix("form_menu") . " m on f.id_form = m.menuid WHERE f.qcm = 0 and m.status =1 ORDER BY m.position";
	...
	while ($row= $xoopsDB->fetchArray($res)) {
		if ( $gperm_handler -> checkRight( 'Droits des categories', $row['id_form'], $groups, $formulaireModule->getVar('mid') ) ) {
			...
			$form['title'] = $myts->displayTarea( str_replace("<br />","", $row['desc_form']), 1 );
