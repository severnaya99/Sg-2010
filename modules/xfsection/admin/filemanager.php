<?php
// $Id: filemanager.php,v 1.4 2006/03/20 03:23:03 ohwada Exp $
...
// 2005-06-20 K.OHWADA
// supress notice: Undefined variable
...
global $xoopsConfig, $xoopsModule, $wfsConfig, $dirlist, $dirpath;

$root = XOOPS_ROOT_PATH; 	//root of webserver
$url = XOOPS_URL; //http of web server
$urlpath = xoops_getenv("PHP_SELF"); //Url for links
...
if ($action == "edit") {
	...
	if ( $confirm ) {
    	//if (xoopsfwrite()) {
			...
			$filename = "{$_POST['workpath']}/{$_POST['fileopened']}";
       		filecheck($_POST['workpath'], $filename, '', 'edit');
			folderwrite($_POST['workpath'], '', '', 'edit');					
			filewrite($_POST['workpath'], $filename, '', 'edit');	
			//safemodeon($_POST['workpath'], '', '', 'edit');	
			
			$filesave = fopen($filename, "w");
			$content = stripcslashes($myts->makeTareaData4Save($_POST['text']));
			fwrite($filesave, $content);
            
			fclose($filesave);
				redirect_header("$urlpath?rootpath=".$_POST['workpath']."",1,"File Saved");
            exit();
		//}
	}else{
		$file = "{$workpath}/{$file}";							
    	...
    	if (is_dir($_POST['file'])) {
			...
			if (!@unlink($_POST['file'])) {
				if (is_file($_POST['file']) && !is_writable($_POST['file'])) {
					...
					if ( ini_get('safe_mode') ) {
						redirect_header("$urlpath?rootpath=".$_POST['workpath']."",1,"Safe Mode restriction applies, not deleted!");
					} else {
						redirect_header("$urlpath?rootpath=".$_POST['workpath']."",1,"unknown error??!, not deleted!");
					}
				}
			} else {
				redirect_header("$urlpath?rootpath=".$_POST['workpath']."",1,"File Deleted");
			}
		}
		exit();
	} else {
...
if ($action == 'mkdir') {
		...
		$newdir = strtolower("{$_POST['workpath']}/{$newdir}");
		//if ($_POST["newtype"] == "dir") {
			$oldumask = umask(0);
			if (@mkdir($newdir, 0777)) {
				redirect_header("$urlpath?rootpath=".$_POST['workpath']."",1,"Folder Created");

			} else {
				redirect_header("$urlpath?rootpath=".$_POST['workpath']."",1,"Error, folder not created");
			}
			umask($oldumask);		
    exit();
}
...
if ($action == 'mkfile') {
		$newfile = str_replace("../", "", $_POST["new_filename"]);
		$newfile = strtolower("{$_POST['workpath']}/{$newfile}");
			if (@touch($newfile)) {
				@chmod($newfile, 0666);
				redirect_header("$urlpath?rootpath=".$_POST['workpath']."",1,"File Created");
			} else {
				redirect_header("$urlpath?rootpath=".$_POST['workpath']."",1,"Error: File not Created");
			}				
    exit();
}
        ...
        if ( $distfilename ) {
            ...
            exit();
			 ...
			 echo "<h4>"._AM_UPLOADFAIL."</h4>";
				...
				if (!file_exists($_POST['uploadpath']) ) echo "Path does not exists";				 
				if (empty($upload->fileName)) echo "No name choosen <br /><br />";
				if (!is_writable($_POST['uploadpath'])) echo $workdir._AM_ISNOTWRITABLE." <br /><br />";
				...
				if (is_file($_POST['uploadpath']."/".$upload->originalName)) echo "The file you are uploading exists on the server.<br /><br />";
...
//Get file info and details
	for ($i=0;$i<Count($filelist);$i++) {
		...
		if (is_editable_file($dirpath."/".$filelist[$i])) 
			...
			$upload = new UploadFile();
            echo $upload->formStart("".$php_self."?action=upload");
            echo $upload->formMax();
            //for($n = 0; $n < $uploads; $n++) {
