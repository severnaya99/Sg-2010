<?php
// $Id: make_link.php,v 1.3 2005/06/30 07:37:15 ohwada Exp $

// 2005-06-30 K.OHWADA
// BUG 6255: Undefined index: page in make_link(), make_pagelink()
...
//	return $_converter->convert($string, ($page != '') ? $page : $vars['page']
	...
	return $_converter->convert($string, $page_1);
}
//インライン要素を置換する
class InlineConverter
{
	...
	function InlineConverter($converters=NULL,$excludes=NULL)
	{
		...
		{
// XFsection
// BracketName, InterWikiName, interwiki
// never use WikiName
			$converters = array(
//				'plugin',        // インラインプラグイン
//				'note',          // 注釈 
...
// XFsection
// suppress only url like http://xxx not braket [[aliase:url]]
...
//	if (!array_key_exists($page,$related) and $page != $vars['page'] and is_page($page))
	...
	if (!array_key_exists($page,$related) and $flag_page and is_page($page))
	{
...
//		return $retval;
//	}
	return "<a href=\"$script$r_page\">$s_alias</a>";
