...
// +----------------------------------------------------------------------+
// $Id: application_top.php 1682 2005-08-02 21:16:56Z drbyte $
//
...
// set the session name and save path
  zen_session_name($xoopsConfig["session_name"]);
...
/*	   if (isset($_POST['xoops_session'])) {
	     zen_session_id($_POST['xoops_session']);
	   } elseif ( ($request_type == 'SSL') && isset($_GET['xoops_session']) ) {
	     zen_session_id($_GET['xoops_session']);
	   } else {
		 zen_session_id($_COOKIE['xoops_session']);
	   }
*/
...
//IMAGINACOLOMBIA
		...
		$language_query = $db->Execute('select * from ' . TABLE_LANGUAGES . ' where directory="'.$language.'"');
		//$language_result = zen_db_fetch_array($language_query); 
		
		$languages_id = $language_query->fields['languages_id'];	
