...
// +----------------------------------------------------------------------+
//  $Id: products_to_categories.php 1592 2005-07-15 20:08:03Z ajeh $
...
//          $cnt_added++;
      ...
      case 'remove_categories_products_to_another_category_linked':
        ...
        if (sizeof($master_categories_id_stop) > 0) {
          ...
          zen_redirect(zen_href_link(FILENAME_PRODUCTS_TO_CATEGORIES, 'products_filter=' . $master_categories_id_stop[0]['products_id']));
...
//          $cnt_removed++;
      ...
      case 'set_master_categories_id':
        ...
        zen_redirect(zen_href_link(FILENAME_PRODUCTS_TO_CATEGORIES, 'products_filter=' . $_GET['products_filter']));
        break;

      case 'update_product':
        // get current master_categories_id for product to compare with final results
        $current_master_categories_id = $db->Execute("select master_categories_id from " . TABLE_PRODUCTS . " where products_id='" . $products_filter . "'");
        ...
        if ($zv_check_master_categories_id == 'true') {
          $db->Execute("update " . TABLE_PRODUCTS . " set master_categories_id='" . $current_master_categories_id->fields['master_categories_id'] . "' where products_id='" . $products_filter . "'");
        } else {
          $db->Execute("update " . TABLE_PRODUCTS . " set master_categories_id=0 where products_id='" . $products_filter . "'");
        }
...
<?php } else { ?>
          ...
          <tr><?php echo zen_draw_form('restrict_product', FILENAME_PRODUCTS_TO_CATEGORIES, '', 'get', '', true) . zen_draw_hidden_field('action', 'set_master_categories_id') . zen_draw_hidden_field('products_filter', $products_filter); ?>
            <td class="main">
              <?php
...
<?php
    ...
    echo zen_draw_form('update', FILENAME_PRODUCTS_TO_CATEGORIES, 'action=update_product&products_filter=' . $products_filter, 'post');
?>
          <tr class="dataTableHeadingRow">
...
<!-- copy products from one category to another as linked or new products -->
            <table border="0" class="dataTableHeadingRow" width="100%">
              ...
              <form name="linked_copy" method="post" action="<?php echo zen_href_link(FILENAME_PRODUCTS_TO_CATEGORIES, 'action=copy_categories_products_to_another_category_linked' . '&products_filter=' . $products_filter, 'NONSSL'); ?>">
              <tr class="dataTableHeadingRow">
...
<!-- remove products from one category that are linked to another category -->
            <table border="0" class="dataTableHeadingRow" width="100%">
              ...
              <form name="linked_remove" method="post" action="<?php echo zen_href_link(FILENAME_PRODUCTS_TO_CATEGORIES, 'action=remove_categories_products_to_another_category_linked' . '&products_filter=' . $products_filter, 'NONSSL'); ?>">
              <tr class="dataTableHeadingRow">
...
<!-- reset master_categories_id to request Categories -->
            <table border="0" class="dataTableHeadingRow" width="100%">
              ...
              <form name="master_reset" method="post" action="<?php echo zen_href_link(FILENAME_PRODUCTS_TO_CATEGORIES, 'action=reset_categories_products_to_another_category_master' . '&products_filter=' . $products_filter, 'NONSSL'); ?>">
              <tr class="dataTableHeadingRow">
...
<?php
  ...
  switch ($action) {
    case 'edit':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . zen_image_submit('button_update.gif', IMAGE_UPDATE) . '&nbsp;<a href="' . zen_href_link(FILENAME_PRODUCTS_TO_CATEGORIES, 'products_filter=' . $products_filter) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>' . '</form>');
      break;
    default:
      ...
      switch (true) {
        case ($product_to_copy->fields['master_categories_id'] == 0 and $products_filter > 0):
          ...
          break;
        default:
          $contents[] = array('text' => '<form action="' . FILENAME_PRODUCTS_TO_CATEGORIES . '.php' . '?action=edit&products_filter=' . $products_filter . '" method="post">');
          ...
          $contents[] = array('align' => 'center', 'text' =>
            '<a href="' . zen_href_link(FILENAME_ATTRIBUTES_CONTROLLER, 'products_filter=' . $products_filter) . '">' . zen_image_button('button_edit_attribs.gif', IMAGE_EDIT_ATTRIBUTES) . '</a>&nbsp;&nbsp;' .
            '<a href="' . zen_href_link(FILENAME_PRODUCTS_PRICE_MANAGER, '&products_filter=' . $products_filter) . '">' . zen_image_button('button_products_price_manager.gif', IMAGE_PRODUCTS_PRICE_MANAGER) . '</a><br /><br />' .
