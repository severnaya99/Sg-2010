...
class ExtcalEventHandler extends ExtcalPersistableObjectHandler {
	...
	function getEventDay($day, $month, $year, $cat=0) {
		...
		$criteriaCompo->setOrder($this->_extcalConfig['sort_order']);
		return $this->getObjects($criteriaCompo);
	}

	// Return the criteria compo object for a week
	function _getEventWeekCriteria($day, $month, $year, $cat) {

	    $user = $GLOBALS['xoopsUser'];

		$userStartTime = mktime(0,0,0,$month,$day,$year);
		$userEndTime = $userStartTime + 604800;
	...
	function getEventWeek($day, $month, $year, $cat=0) {
	...
	function getThisDayEvent($nbEvent, $cat = 0) {
		...
		$this->_addCatSelectCriteria($criteriaCompo, $cat);
		$this->_addCatPermCriteria($criteriaCompo, $GLOBALS['xoopsUser']);
		$criteriaCompo->add(new Criteria('event_start', $dayStart, '>='));
	...
	function getNewEvent($nbEvent, $cat = 0, $skipPerm = false) {
		$criteriaCompo = new CriteriaCompo();
		$this->_addCatSelectCriteria($criteriaCompo, $cat);
		if(!$skipPerm)
			$this->_addCatPermCriteria($criteriaCompo, $GLOBALS['xoopsUser']);
	...
	function getRecurRules($parm) {
		...
		switch($recurFreq) {
			...
			case 'monthly':
				...
				if(isset($parm['rrule_monthly_byday'])) {
	...
	function getRecurEnd($data, $parm) {
		...
		switch($recurFreq) {
			...
			case 'weekly':
				...
				$offset = date('w',$startWeekTS) - $xoopsModuleConfig['week_start_day'];
				$startWeekTS = $startWeekTS - ($offset * 86400);

				$recurEnd = $startWeekTS + ($parm['rrule_weekly_interval'] * 604800) - 1;
	...
	function getRecurEventToDisplay(&$event, $periodStart, $periodEnd) {
		...
		switch($eventOptions[0]) {
			...
			case 'daily':
				//echo "Daily occur<br />";
				array_shift($eventOptions);
				$rrule_daily_interval = $eventOptions[0];
				...
				while($nbOccur < $rrule_daily_interval) {
					//echo "Occur $nbOccur<br />";
					//echo date("d/m/y H:i", $occurEventStart)." <= ".date("d/m/y H:i", $periodStart)." && ".date("d/m/y H:i", $occurEventEnd)." >= ".date("d/m/y H:i", $periodStart)."<br />";
					//echo date("d/m/y H:i", $occurEventStart)." <= ".date("d/m/y H:i", $periodEnd)." && ".date("d/m/y H:i", $occurEventEnd)." >= ".date("d/m/y H:i", $periodEnd)."<br />";
					//echo date("d/m/y H:i", $occurEventStart)." <= ".date("d/m/y H:i", $periodEnd)." && ".date("d/m/y H:i", $occurEventEnd)." >= ".date("d/m/y H:i", $periodEnd)."<br />";
					//echo date("d/m/y H:i", $occurEventStart)." >= ".date("d/m/y H:i", $periodStart)." && ".date("d/m/y H:i", $occurEventEnd)." <= ".date("d/m/y H:i", $periodEnd)."<br />";
					// Add this event occurence only if it's on the period view
					if(
						// Event start before and finish during
						($occurEventStart <= $periodStart && $occurEventEnd >= $periodStart) ||
						// Event start during and finish after
						($occurEventStart <= $periodEnd && $occurEventEnd >= $periodEnd) ||
						// Event start before and finish after
						($occurEventStart <= $periodEnd && $occurEventEnd >= $periodEnd) ||
						// Event start and finish during
						($occurEventStart >= $periodStart && $occurEventEnd <= $periodEnd)) {
			...
			case 'weekly':
				...
				array_shift($eventOptions);
				$rrule_weekly_interval = $eventOptions[0];
				array_shift($eventOptions);
				...
				// Parse all occurence of this event
				while($nbOccur < $rrule_weekly_interval) {
					//echo "Occur $nbOccur<br />";
					//echo date("d/m/y H:i", $occurEventStart)." <= ".date("d/m/y H:i", $periodStart)." && ".date("d/m/y H:i", $occurEventEnd)." >= ".date("d/m/y H:i", $periodStart)."<br />";
					//echo date("d/m/y H:i", $occurEventStart)." <= ".date("d/m/y H:i", $periodEnd)." && ".date("d/m/y H:i", $occurEventEnd)." >= ".date("d/m/y H:i", $periodEnd)."<br />";
					//echo date("d/m/y H:i", $occurEventStart)." <= ".date("d/m/y H:i", $periodEnd)." && ".date("d/m/y H:i", $occurEventEnd)." >= ".date("d/m/y H:i", $periodEnd)."<br />";
					//echo date("d/m/y H:i", $occurEventStart)." >= ".date("d/m/y H:i", $periodStart)." && ".date("d/m/y H:i", $occurEventEnd)." <= ".date("d/m/y H:i", $periodEnd)."<br />";
					//echo $dayArray[date('w', $occurEventStart)]."<br />";
			...
			case 'monthly':
				...
				array_shift($eventOptions);
				$rrule_monthly_interval = $eventOptions[0];
				array_shift($eventOptions);
				...
				// Parse all occurence of this event
				while($nbOccur < $rrule_monthly_interval) {
					
					$eventDayOccurStart = $this->_getOccurTS($month, $year, $eventOptions[0]);
					
					$eventHourOccurStart = $event['event_start'] - mktime(0,0,0,date('n',$event['event_start']),date('j',$event['event_start']),date('Y',$event['event_start']));
					$eventHourOccurEnd = $event['event_end'] - $event['event_start'];
					$event['event_start'] = $eventDayOccurStart + $eventHourOccurStart;
					$event['event_end'] = $eventDayOccurStart + $eventHourOccurEnd;

					if($event['event_start'] > $periodEnd) {
						break;
					}
					$recuEvents[] = $event;
			...
			case 'yearly':
				...
				while($nbOccur < $rrule_monthly_interval) {
					...
					$eventHourOccurStart = $event['event_start'] - mktime(0,0,0,date('n',$event['event_start']),date('j',$event['event_start']),date('Y',$event['event_start']));
					$eventHourOccurEnd = $eventHourOccurStart + $event['event_end'] - $event['event_start'];
					//echo date("d/m/y H:i", $event['event_start'])."<br />";
					//echo date("d/m/y H:i", $event['event_end'])."<br />";
					//echo "<b>$eventHourOccurEnd</b><br /><br />";
					$event['event_start'] = $eventDayOccurStart + $eventHourOccurStart;
					$event['event_end'] = $eventDayOccurStart + $eventHourOccurEnd;

					
					//echo date("d/m/y H:i", $event['event_start'])." <= ".date("d/m/y H:i", $periodStart)." && ".date("d/m/y H:i", $event['event_end'])." >= ".date("d/m/y H:i", $periodStart)."<br />";
					//echo date("d/m/y H:i", $event['event_start'])." <= ".date("d/m/y H:i", $periodEnd)." && ".date("d/m/y H:i", $event['event_end'])." >= ".date("d/m/y H:i", $periodEnd)."<br />";
					//echo date("d/m/y H:i", $event['event_start'])." <= ".date("d/m/y H:i", $periodEnd)." && ".date("d/m/y H:i", $event['event_end'])." >= ".date("d/m/y H:i", $periodEnd)."<br />";
					//echo date("d/m/y H:i", $event['event_start'])." >= ".date("d/m/y H:i", $periodStart)." && ".date("d/m/y H:i", $event['event_end'])." <= ".date("d/m/y H:i", $periodEnd)."<br />";
					//echo $month."<br />";
					
					if(
						// Event start before and finish during
						(($event['event_start'] <= $periodStart && $event['event_end'] >= $periodStart) ||
						// Event start during and finish after
						($event['event_start'] <= $periodEnd && $event['event_end'] >= $periodEnd) ||
						// Event start before and finish after
						($event['event_start'] <= $periodEnd && $event['event_end'] >= $periodEnd) ||
						// Event start and finish during
						($event['event_start'] >= $periodStart && $event['event_end'] <= $periodEnd)) &&
						// This week day is selected
						in_array($month, $eventOptions)) {

						$recuEvents[] = $event;
					} elseif($event['event_start'] > $periodEnd) {
						break;
					}
	...
	function getSearchEvent($queryarray, $andor, $limit, $offset, $userId, $user)
	{
		global $xoopsDB;
	
		$sql = "SELECT event_id, event_title, event_submitter, event_submitdate FROM ".$xoopsDB->prefix("extcal_event")." WHERE event_approved = '1'";
		$authorizedAccessCats = $this->_extcalPerm->getAuthorizedCat($user, 'extcal_cat_view');
		$count = count($authorizedAccessCats);
		if($count > 0) {
			$in = '('.$authorizedAccessCats[0];
