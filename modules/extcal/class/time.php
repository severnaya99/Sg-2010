	...
	function getFormatedReccurRule($event_recur_rules) {
		...
		switch($eventOptions[0]) {
			...
			case 'daily':
				...
				return "Each day during $interval days";
			...
			case 'monthly':
				...
				if(substr($eventOptions[2], 0,2) == 'MD') {
					return "Each month, the ".substr($eventOptions[2], 2)." during $interval month";
				} else {
					return "Each month, the ".$eventOptions[2]." during $interval month";
				}
			...
			case 'yearly':
				...
				$ret = "Each year, on";
				foreach($eventOptions as $option) {
					$ret .= " ".$monthName[$option].", ";
				}
				$ret .= "the $monthDays[$day], during $interval years";
