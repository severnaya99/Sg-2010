...
if($xoopsUser && $xoopsModuleConfig['whos_going']) {
	...
	if(($_POST['mode'] == 'add' || $_POST['mode'] == 'remove') && $_POST['event'] > 0) {
		...
		if($_POST['mode'] == 'add') {
			...
			if($event->getVar('event_nbmember') > 0 && $eventmemberHandler->getNbMember($_POST['event']) >= $event->getVar('event_nbmember')) {
				$rediredtMessage = _MD_EXTCAL_MAX_MEMBER_REACHED;
			} else {
				$eventmemberHandler->createEventmember(array('event_id'=>$_POST['event'], 'uid'=>$xoopsUser->getVar('uid')));
				$rediredtMessage = _MD_EXTCAL_WHOS_GOING_ADDED_TO_EVENT;
			}
		// If the user have to be remove
		} else if($_POST['mode'] == 'remove') {
			$eventmemberHandler->deleteEventmember(array($_POST['event'], $xoopsUser->getVar('uid')));
