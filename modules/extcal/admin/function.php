...
function adminMenu($currentoption = 0, $breadcrumb = '')
{
	global $xoopsConfig;
	global $xoopsModule;
	if(file_exists(XOOPS_ROOT_PATH.'/modules/extcal/language/'.$xoopsConfig['language'].'/modinfo.php')) {
		include_once XOOPS_ROOT_PATH.'/modules/extcal/language/'.$xoopsConfig['language'].'/modinfo.php';
	} else {
		include_once XOOPS_ROOT_PATH.'/modules/extcal/language/english/modinfo.php';
	}
	...
	echo "<a href=\"../index.php\">"._AM_EXTCAL_GO_TO_MODULE."</a> | <a href=\"".XOOPS_URL."/modules/system/admin.php?fct=preferences&amp;op=showmod&amp;mod=".$xoopsModule->getVar('mid')."\">"._AM_EXTCAL_PREFERENCES."</a>\n";
	echo "</td>\n";
	echo "<td style=\"width: 30%; font-size: 10px; text-align: right; color: #2F5376; padding: 0 6px; line-height: 18px;\">\n";
	echo "<b>".$xoopsModule->getVar('name')."&nbsp;"._AM_EXTCAL_ADMINISTRATION."</b>&nbsp;".$breadcrumb."\n";
	echo "</td>\n";
	echo "</tr>\n";
	echo "</table>\n";
	echo "</div>\n";
	...
	foreach($adminmenu as $key=>$link) {
		...
		echo "<a href=\"".XOOPS_URL."/modules/extcal/".$link['link']."\"><span>".$link['title']."</span></a>\n";
		echo "</li>\n";
	}
	echo "</ul>\n";
	echo "</div>\n";
