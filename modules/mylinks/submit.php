<?php
// $Id: submit.php,v 1.17 2004/12/26 19:11:57 onokazu Exp $
//  ------------------------------------------------------------------------ //
...
if (!empty($_POST['submit'])) {
	...
	if (empty($xoopsUser)) {
	  if ($_POST["security"]=="") {
       	include_once XOOPS_ROOT_PATH."/header.php";
			  ...
			  echo "Error Code: 0000<br /><br /><br />";
			  echo "<b>ERROR:</b> "._MYLINKS_SECURITY_GETCODE."<br /><br /><br />";
		  ...
		  $datekey = date("F j");
      $rcode = hexdec(md5($_SERVER['HTTP_USER_AGENT'].$_POST['securityHidden'].$datekey));
      $code = substr($rcode, 2, 6);
			if ($code != $_POST["security"]) {
			  include_once XOOPS_ROOT_PATH."/header.php";
			  //OpenTable();
			  echo "<div><b>".$xoopsConfig['sitename']." Error</b><br /><br />";
			  echo "Error Code: 0000<br /><br /><br />";
			  echo "<b>ERROR:</b> "._MYLINKS_SECURITY_ERROR."<br /><br /><br />";
			  echo "[ <a href='javascript:history.go(-1)'>Go Back</a> ]</div>";
			  //CloseTable();
			  include_once XOOPS_ROOT_PATH."/footer.php";
				exit;
    ...
    $security = "<img src=".XOOPS_URL."/modules/".$xoopsModule->dirname()."/getgfx.php?random_num=$random_num border='1' alt='"._MYLINKS_SECURITY_CODE."' title='"._MYLINKS_SECURITY_CODE."'>";		 
	  $xoopsTpl->assign('security',$security);
	  $xoopsTpl->assign('securitylabel',_MYLINKS_SECURITY_CODE);
		$xoopsTpl->assign('securityHidden','<input type="hidden" name="securityHidden" value="'.$random_num.'">');
	}
