<?php
// $Id: catmgr.php,v 1.5 2006/10/02 12:07:08 mcleines Exp $
//  ------------------------------------------------------------------------ //
...
function display_cat_list()
{
        ...
        foreach ($CAT_LIST3 as $key => $category){
                ...
                if ($category['cid'] != 1) {
                        echo '                <td class="odd" width="4%"><a href="'.$PHP_SELF.'?op=deletecat&cid='.$category['cid'].'" onClick="return confirmDel(\''.str_replace('&nbsp;','', $myts->makeTboxData4Show($category['name'])).'\')">'.'<img src="../images/delete.gif"  border="0">'.'</a></td>'."\n";
                } else {
                        echo '                <td class="odd" width="4%">'.'&nbsp;'.'</td>'."\n";
                }
...
switch($op){
        ...
        $name = trim($_POST['name']) ? $myts->makeTboxData4Save($_POST['name']) : '&lt;???&gt;';
...
$current_category['name']= $myts->makeTboxData4Edit($current_category['name']);
$current_category['description']= $myts->makeTareaData4Edit($current_category['description']);
echo <<<EOT
    <table border='0' cellpadding='0' cellspacing='1' width='100%' class='outer'>
    <tr><th colspan="2">
EOT;
