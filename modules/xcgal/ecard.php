<?php
// $Id: ecard.php,v 1.7 2006/10/02 12:07:08 mcleines Exp $
//  ------------------------------------------------------------------------ //
...
function send_ecard($recipient_email, $recipient_name, $greetings, $msg_content, $sender_name, $sender_email, $image, $n_picname, $redirect_link)
{
    ...
    $s_time = time()-3600;
    $result = $xoopsDB->query("SELECT * from ".$xoopsDB->prefix("xcgal_ecard")." WHERE (sess_id ='".session_id()."' || sender_email = '".$myts->makeTboxData4Save($sender_email)."' || sender_ip ='".$_SERVER['REMOTE_ADDR']."' ".$s_uid.") AND s_time > ".$s_time."");
    ...
    $ecardText= str_replace("{CARD_LINK}", XOOPS_URL."/modules/xcgal/displayecard.php?data=".$e_id , $ecardText);    $xoopsMailer->setToEmails($recipient_email);
...
function build_html_card($sender_name, $sender_email, $n_picname, $message, $greetings,$e_id){
    global $myts, $xoopsConfig, $xoopsModuleConfig;
        if(!stristr($n_picname, 'http:')) $n_picname = XOOPS_URL."/modules/xcgal/".$n_picname;
    ...
    require_once XOOPS_ROOT_PATH.'/class/template.php';
        ...
        $ecardTpl->assign('ecard_title', sprintf(_MD_CARD_ECARD_TITLE, $myts->makeTboxData4Show($sender_name)));
        ...
        $ecardTpl->assign('greetings',$myts->makeTboxData4Show($greetings));
        $ecardTpl->assign('message',$msg_content);
        $ecardTpl->assign('sender_email',$myts->makeTboxData4Show($sender_email));
        $ecardTpl->assign('sender_name',$myts->makeTboxData4Show($sender_name));
        ...
        $ecardTpl->assign('xoops_url', XOOPS_URL);
