<?php
// $Id: xcgal_blocks.php,v 1.3 2005/12/16 14:53:56 mcleines Exp $
//  ------------------------------------------------------------------------ //
                ...
                foreach ($rowset as $subcat){
                        if($pos>0){
                                $CAT_LIST_BLOCK[]=array(
                                        ...
                                        'prev' => $prev_cid,
                                        'cat_count' => $cat_count,
                                        'name' => $ident.$myts->makeTboxData4Show($subcat['name']));
    ...
    $select_columns = 'pid, filepath, filename, url_prefix, filesize, pwidth, pheight, ctime';
        ...
        case '5': // Last comments
                ...
                if ($set_caption) foreach ($rowset as $key => $row){
                        ...
                        $comtitle='<a href="'.XOOPS_URL.'/modules/xcgal/'.$comment_config['pageName'].'?'.$comment_config['itemName'].'='.$row['com_itemid'].'&amp;com_id='.$row['com_id'].'&amp;com_rootid='.$row['com_rootid'].'&amp;com_mode=flat&amp;'.$row['com_exparams'].'#comment'.$row['com_id'].'">'.$row['com_title'].'</a>';
        ...
        case '4': // Top rated pictures
                ...
                if ($set_caption) foreach ($rowset as $key => $row){
                        $caption = "<span style=\" font-weight : bold; font-size: 10px; padding: 2px; display : block;\">".'<img src="'.XOOPS_URL.'/modules/xcgal/images/rating'.round($row['pic_rating']/2000).'.gif" align="absmiddle"/>'.'<br />'.sprintf(_MB_FUNC_VOTE, $row['votes']).'</span>';
                        $rowset[$key]['caption_text'] = $caption;
                }
                return $rowset;
                break;
