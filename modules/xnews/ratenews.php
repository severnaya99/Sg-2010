...
$ratenews=nw_getmoduleoption('ratenews', NW_MODULE_DIR_NAME);
if(!$ratenews) {
...
if(!empty($_POST['submit'])) {			// The form was submited
	...
	if ($rating=='--') {
		redirect_header(NW_MODULE_URL . '/ratenews.php?storyid='.$storyid,4,_MA_NW_NORATING);
		exit();
	}
	...
	if ($ratinguser != 0) {
		...
		while(list($ratinguserDB)=$xoopsDB->fetchRow($result)) {
			if ($ratinguserDB==$ratinguser) {
				redirect_header(NW_MODULE_URL . '/article.php?storyid='.$storyid,4,_MA_NW_CANTVOTEOWN);
				exit();
			}
		}
		...
		while(list($ratinguserDB)=$xoopsDB->fetchRow($result)) {
			if ($ratinguserDB==$ratinguser) {
				redirect_header(NW_MODULE_URL . '/article.php?storyid='.$storyid,4,_MA_NW_VOTEONCE);
				exit();
			}
		}
	...
	redirect_header(NW_MODULE_URL . '/article.php?storyid='.$storyid, 4, $ratemessage);
	exit();
    ...
    if(is_object($news)) {
		...
		redirect_header(NW_MODULE_URL . '/index.php', 3, _ERRORS);
		exit();
    }
    //DNPROSSI - ADDED
