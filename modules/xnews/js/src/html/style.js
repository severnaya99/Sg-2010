...
dojo.html._insertedCsnw_sFiles = []; // cache container needed because IE reformats cssText when added to DOM
dojo.html.insertCssFile = function(/* string */URI, /* HTMLDocument? */doc, /* boolean? */checkDuplicates, /* boolean */fail_ok){
	...
	if(checkDuplicates){
		var idx = -1, node, ent = dojo.html._insertedCsnw_sFiles;
		...
		if(node){
			...
			// delete this entry
			dojo.html._insertedCsnw_sFiles.shift(idx, 1);
		}
	}

	var style = dojo.html.insertCssText(cssStr, doc);
	dojo.html._insertedCsnw_sFiles.push({'doc': doc, 'cssText': cssStr, 'nodeRef': style});
