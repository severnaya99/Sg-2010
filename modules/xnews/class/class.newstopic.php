...
class nw_NewsTopic extends XoopsTopic
{
	...
	function nw_NewsTopic($topicid=0)
	{
	...
	function MakeMyTopicSelBox($none=0, $seltopic=-1, $selname="", $onchange="", $checkRight = false, $perm_type='nw_view')
	{
	    $perms='';
	    if ($checkRight) {
	        global $xoopsUser;
	...
	function getAllTopicsCount($checkRight = true)
	{
	    ...
	    if ($checkRight) {
	        ...
	        $groups = is_object($xoopsUser) ? $xoopsUser->getGroups() : XOOPS_GROUP_ANONYMOUS;
	        $gperm_handler =& xoops_gethandler('groupperm');
	        $topics = $gperm_handler->getItemIds('nw_submit', $groups, $newsModule->getVar('mid'));
	        if(count($topics)>0) {
	        	$topics = implode(',', $topics);
	...
	*/
	...
	function getTopicTitleFromId($topic,&$topicstitles)
	{
	    ...
	    $result = $this->db->query($sql);
		while ($row = $this->db->fetchArray($result)) {
			$topicstitles[$row["topic_id"]]=array("title"=>$myts->displayTarea($row["topic_title"]),"picture"=>NW_TOPICS_FILES_URL.'/'.$row["topic_imgurl"]);
		}
		return $topicstitles;
	}
