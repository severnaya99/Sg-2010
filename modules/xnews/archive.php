<?php
// $Id: archive.php,v 1.5 2004/07/09 13:49:50 hthouzard Exp $
//  ------------------------------------------------------------------------ //
	...
	if(is_array($storyarray) && $count>0) {
		foreach ($storyarray as $article) {
			...
			if ( $seo_enabled == 1 ) {
				...
				$story['print_link'] = NW_MODULE_URL . '/articles.print.' . $article->storyid() . '/' .$print_item;
			} else {
	    		$story['title'] = "<a href='". NW_MODULE_URL . '/index.php?storytopic=' . $article->topicid() . "'>" . $article->topic_title() . "</a>: <a href='" . NW_MODULE_URL . "/article.php?storyid=" . $article->storyid() . "'" . $htmltitle.">" . $article->title() . "</a>";
				$story['print_link'] = NW_MODULE_URL . '/print.php?storyid=' . $article->storyid();
			}
