...
switch ($op)
{
    case "delvotes":
        ...
        $sql = $xoopsDB->queryF("DELETE FROM " . $xoopsDB->prefix('xf_votedata') . " WHERE ratingid = $rid");
        $xoopsDB->query($sql);
    ...
    default:
		...
		$sql = "SELECT * FROM " . $xoopsDB->prefix('xf_votedata') . " ORDER BY ratingtimestamp DESC";
        $results = $xoopsDB->query($sql, 20, $start);
		$votes = $xoopsDB->getRowsNum($results);

        $sql = "SELECT rating FROM " . $xoopsDB->prefix('xf_votedata') . "";
