...
if (!empty($_POST['submit'])) {
    ...
    $sql = "SELECT t.topic_id FROM " . $xoopsDB->prefix("xf_topics") . " t, " . $xoopsDB->prefix("xf_posts") . "  p
    ...
    if ($topic_list != null) {
        $sql = "SELECT post_id FROM " . $xoopsDB->prefix("xf_posts") . "
    ...
    if ($post_list != null) {
        ...
        if ($store != null) {
            $sql = "UPDATE " . $xoopsDB->prefix("xf_posts") . " SET forum_id=$store WHERE topic_id IN ($topic_list)";
            if (!$result = $xoopsDB->query($sql)) {
                return _AM_XFORUM_ERROR;
            }

            $sql = "UPDATE " . $xoopsDB->prefix("xf_topics") . " SET forum_id=$store WHERE topic_id IN ($topic_list)";
            if (!$result = $xoopsDB->query($sql)) {
                return _MD_ERROR;
            }
        } else {
            // ARCHIVING POSTS
            if ($archive == 1) {
                $result = $xoopsDB->query("SELECT p.topic_id, p.post_id, t.post_text FROM " . $xoopsDB->prefix("xf_posts") . " p, " . $xoopsDB->prefix("xf_posts_text") . " t WHERE p.post_id IN ($post_list) AND p.post_id=t.post_id");
                while (list($topic_id, $post_id, $post_text) = $xoopsDB->fetchRow($result)) {
                    $sql = $xoopsDB->query("INSERT INTO " . $xoopsDB->prefix("xf_archive") . " (topic_id, post_id, post_text) VALUES ($topic_id, $post_id, '$post_text')");
                }
            }
            // DELETE POSTS
            $sql = "DELETE FROM " . $xoopsDB->prefix("xf_posts") . " WHERE topic_id IN ($topic_list)";
            if (!$result = $xoopsDB->query($sql)) {
                return _MD_ERROR;
            }
            // DELETE TOPICS
            $sql = "DELETE FROM " . $xoopsDB->prefix("xf_topics") . " WHERE topic_id IN ($topic_list)";
            if (!$result = $xoopsDB->query($sql)) {
                return _MD_ERROR;
            }
            // DELETE POSTS_TEXT
            $sql = "DELETE FROM " . $xoopsDB->prefix("xf_posts_text") . " WHERE post_id IN ($post_list)";
            if (!$result = $xoopsDB->query($sql)) {
                return _MD_ERROR;
            }
	...
	*/
    // $sql="SELECT p.topic_id, p.post_id t.post_text FROM ".$xoopsDB->prefix("xf_posts")." p, ".$xoopsDB->prefix("xf_posts_text")." t WHERE p.post_id IN ($post_list) AND p.post_id=t.post_id";
    ...
    $sql = "SELECT forum_name, forum_id FROM " . $xoopsDB->prefix("xf_forums") . " ORDER BY forum_id";
    if ($result = $xoopsDB->query($sql)) {
