<?php
// $Id: polls.php,v 4.04 2008/06/05 15:35:59 wishcraft Exp $
//  ------------------------------------------------------------------------ //
...
if($isadmin){
	$perm = true;
}elseif ($topic_handler->getPermission($viewtopic_forum, $xforumtopic->getVar('topic_status'), "addpoll")
	&& $viewtopic_forum->getVar('allow_polls') == 1
){
	if( ($op=="add" || $op=="save") && 
		!$xforumtopic->getVar("topic_haspoll") &&
		is_object($xoopsUser) && $xoopsUser->getVar("uid")==$xforumtopic->getVar("topic_poster")
	){ 
		$perm = true;
	}
...
if(!$perm){
    redirect_header("viewtopic.php?topic_id=".$topic_id."&forum=".$viewtopic_forum->getVar('forum_id'),2,_MD_NORIGHTTOACCESS);
}
...
if ( $op == "save" ) {
	...
	if ( !empty($new_poll_id) ) {
		...
		$sql = "UPDATE ".$xoopsDB->prefix("xf_topics")." SET topic_haspoll = 1, poll_id = $new_poll_id WHERE topic_id = $topic_id";
...
if ( $op == "delete_ok" ) {
	...
	if ( $poll->delete() != false ) {
		...
		$sql = "UPDATE ".$xoopsDB->prefix("xf_topics")." SET votes = 0, topic_haspoll = 0, poll_id = 0 WHERE topic_id = $topic_id";
        if ( !$result = $xoopsDB->query($sql) ) {
        	forum_message("poll removal from topic error: ".$sql);
        }
	}
	redirect_header("viewtopic.php?topic_id=$topic_id",1,_MD_POLL_DBUPDATED);
	//exit();
