...
if ( isset($_POST['submit']) ) {
	...
	if('delete'==$mode){
    	...
    	$sql = sprintf("DELETE FROM %s WHERE topic_id = %u", $xoopsDB->prefix("xf_topics"), $topic_id);
        $result = $xoopsDB->queryF($sql);
        
        $sql = sprintf("DELETE FROM %s WHERE topic_id = %u", $xoopsDB->prefix("xf_votedata"), $topic_id);
        $result = $xoopsDB->queryF($sql);

        $sql = sprintf("UPDATE %s SET forum_topics = forum_topics-1 WHERE forum_id = %u", $xoopsDB->prefix("xf_forums"), $forum);
        $result = $xoopsDB->queryF($sql);
        ...
        if ($newforum > 0) {
            $sql = sprintf("UPDATE %s SET forum_id = %u WHERE topic_id = %u", $xoopsDB->prefix("xf_topics"), $newforum, $topic_id);
            if ( !$r = $xoopsDB->query($sql) ) {
	            return false;
            }
            $sql = sprintf("UPDATE %s SET forum_id = %u WHERE topic_id = %u", $xoopsDB->prefix("xf_posts"), $newforum, $topic_id);
            if ( !$r = $xoopsDB->query($sql) ) {
	            return false;
            }
        ...
        $sql = sprintf("UPDATE %s SET ".$action[$mode]['sql']." WHERE topic_id = %u", $xoopsDB->prefix("xf_topics"), $topic_id);
        if ( !$r = $xoopsDB->query($sql) ) {
