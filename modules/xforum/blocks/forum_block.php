...
function b_forum_array_filter($var){
	return $var > 0;
}
...
function b_forum_topic_show($options)
{
    global $xoopsConfig;
    global $access_forums;
...
function b_forum_edit($options)
{
    ...
    $options_forum = array_filter(array_slice($options, 6), "b_forum_array_filter"); // get allowed forums
...
function b_forum_topic_edit($options)
{
    $form  = _MB_XFORUM_CRITERIA."<select name='options[0]'>";
    $form .= "<option value='time'";
	    if($options[0]=="time") $form .= " selected='selected' ";
...
function b_forum_post_edit($options)
{
    $form  = _MB_XFORUM_CRITERIA."<select name='options[0]'>";
