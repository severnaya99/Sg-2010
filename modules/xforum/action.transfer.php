...
if(!$approved = $xforumtopic->getVar('approved'))    die(_NOPERM);
...
if (!$topic_handler->getPermission($viewtopic_forum, $xforumtopic->getVar('topic_status'), "view"))   die(_NOPERM);
...
if(empty($_POST["op"])){
	...
	switch($op){
	    case "pdf":
			$data['subtitle'] = $xforumtopic->getVar('topic_title');
		    break;
