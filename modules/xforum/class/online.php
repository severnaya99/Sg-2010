...
class xforumOnlineHandler
{
    ...
    function init($xforum = null, $xforumtopic = null)
    {
        ...
        if (is_object($xforumtopic)) {
            $this->forumtopic = $xforumtopic->getVar('topic_id');
            if(empty($this->forum))  $this->forum = $xforumtopic->getVar('forum_id');
        } else {
            $this->forumtopic = intval($xforumtopic);
        }
