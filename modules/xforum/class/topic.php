...
class xforumTopicHandler extends ArtObjectHandler
{
    function xforumTopicHandler(&$db) {
        $this->ArtObjectHandler($db, 'xf_topics', 'Topic', 'topic_id', 'topic_title');
    }
    ...
    function &getPostTree(&$postArray, $pid=0)
    {
        ...
        $xforumTree->getPostTree($postsArray, $pid);
        return $postsArray;
    }
    ...
    function synchronization($object = null, $force = true)
    {
        ...
        $sql = "SELECT MIN(post_id) AS top_post FROM ".$xoopsDB->prefix("xf_posts")." WHERE approved = 1 AND topic_id = ".$object->getVar("topic_id");
        if ( $result = $xoopsDB->query($sql) ) {
        	list($top_post) = $xoopsDB->fetchRow($result);
        	if(empty($top_post)) return false;
	        $sql = 	"UPDATE ".$xoopsDB->prefix("xf_posts").
	        		" SET pid = 0 ".
		    ...
		    // MYSQL syntax error
	        $sql = 	"UPDATE ".$xoopsDB->prefix("xf_posts").
	        		" SET ".$xoopsDB->prefix("xf_posts"). ".pid = ".$top_post.
	        		" LEFT JOIN ".$xoopsDB->prefix("xf_posts"). " AS bb".
	        		" 	ON bb.post_id = ".$xoopsDB->prefix("xf_posts"). ".pid".
	        		" WHERE".
	        		" 	".$xoopsDB->prefix("xf_posts"). ".topic_id = ".$object->getVar("topic_id").
	        		" 	AND ".$xoopsDB->prefix("xf_posts"). ".post_id <> ".$top_post.
