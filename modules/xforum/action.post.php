...
switch($op){
	case "restore":
		...
		foreach(array_keys($xforums) as $xforum){
			$forum_handler->synchronization($xforum);
		}
		break;
	case "approve":
		$post_id = array_values($post_id);
	...
	case "split":
		...
		/* split a post and its children posts */
	        ...
	        $mytree = new XoopsTree($xoopsDB->prefix("xf_posts"), "post_id", "pid");
        ...
        $sql = sprintf("UPDATE %s SET forum_topics = forum_topics+1 WHERE forum_id = %u", $xoopsDB->prefix("xf_forums"), $forum_id);
