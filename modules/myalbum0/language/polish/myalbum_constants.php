<?php
//Polish translation by kaper@zk-p.pl


if( defined( 'FOR_XOOPS_LANG_CHECKER' ) || ! defined( 'MYALBUM_CNST_LOADED' ) ) {


// Appended by Xoops Language Checker -GIJOE- in 2005-08-31 15:23:37
define('GPERM_TELLAFRIEND','1024');
define('_ALBM_GPERM_G_TELLAFRIEND','Tell a friend');

define( 'MYALBUM_CNST_LOADED' , 1 ) ;

// System Constants (Don't Edit)
define( "GPERM_INSERTABLE" , 1 ) ;
define( "GPERM_SUPERINSERT" , 2 ) ;
define( "GPERM_EDITABLE" , 4 ) ;
define( "GPERM_SUPEREDIT" , 8 ) ;
define( "GPERM_DELETABLE" , 16 ) ;
define( "GPERM_SUPERDELETE" , 32 ) ;
define( "GPERM_TOUCHOTHERS" , 64 ) ;
define( "GPERM_SUPERTOUCHOTHERS" , 128 ) ;
define( "GPERM_RATEVIEW" , 256 ) ;
define( "GPERM_RATEVOTE" , 512 ) ;

// Global Group Permission
define( "_ALBM_GPERM_G_INSERTABLE" , "Wysy³anie (do zatwierdzenia)" ) ;
define( "_ALBM_GPERM_G_SUPERINSERT" , "Super Wysy³anie" ) ;
define( "_ALBM_GPERM_G_EDITABLE" , "Edycja (do zatwierdzenia)" ) ;
define( "_ALBM_GPERM_G_SUPEREDIT" , "Super Edycja" ) ;
define( "_ALBM_GPERM_G_DELETABLE" , "Usuwanie (do zatwierdzenia)" ) ;
define( "_ALBM_GPERM_G_SUPERDELETE" , "Super Usuwanie" ) ;
define( "_ALBM_GPERM_G_TOUCHOTHERS" , "Touch photos posted by others" ) ;
define( "_ALBM_GPERM_G_SUPERTOUCHOTHERS" , "Super Touch others" ) ;
define( "_ALBM_GPERM_G_RATEVIEW" , "Przegl±danie ocen" ) ;
define( "_ALBM_GPERM_G_RATEVOTE" , "Ocenianie" ) ;

// Caption
define( "_ALBM_CAPTION_TOTAL" , ": " ) ; // Total number of photos inside main category
define( "_ALBM_CAPTION_GUESTNAME" , "Nieznajomy" ) ;
define( "_ALBM_CAPTION_REFRESH" , "Od¶wie¿" ) ;
define( "_ALBM_CAPTION_IMAGEXYT" , "Size(Type)" ) ;
define( "_ALBM_CAPTION_CATEGORY" , "Category" ) ;

}

?>
