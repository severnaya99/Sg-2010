<?php
// $Id: xoopspoll.php,v 1.12 2004/12/26 19:12:12 onokazu Exp $
//  ------------------------------------------------------------------------ //
...
class XoopsPoll extends XoopsObject
{
	...
	function XoopsPoll($id=null)
	{
		...
		$this->initVar("display", XOBJ_DTYPE_INT, 1, false);
                $this->initVar("anonymous", XOBJ_DTYPE_INT, 1, false);
	...
	function store()
	{
		...
		if ( empty($poll_id) ) {
			...
			$sql = "INSERT INTO ".$this->db->prefix("xoopspoll_desc")." (poll_id, question, description, user_id, start_time, end_time, votes, voters, display, anonymous, weight, multiple, mail_status) VALUES ($poll_id, ".$this->db->quoteString($question).", ".$this->db->quoteString($description).", $user_id, $start_time, $end_time, 0, 0, $display, $anonymous, $weight, $multiple, $mail_status)";
		} else {
			$sql ="UPDATE ".$this->db->prefix("xoopspoll_desc")." SET question=".$this->db->quoteString($question).", description=".$this->db->quoteString($description).", start_time=$start_time, end_time=$end_time, display=$display, anonymous=$anonymous, weight=$weight, multiple=$multiple, mail_status=$mail_status WHERE poll_id=$poll_id";
		}
		//echo $sql;
	...
	function hasExpired()
	{
     ...
     function isAllowedToVote()
     {
            ...
            if ($this->getVar("anonymous") == 0 && !is_object($xoopsUser) ){
                return false;
            }
            return true;
     }
