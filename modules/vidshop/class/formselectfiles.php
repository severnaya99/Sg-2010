 ...
 * @access 		public 
 */
class VidshopFormSelectFiles extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function VidshopFormSelectFiles($caption, $name, $value = null, $size = 1, $multiple = false)
    {
    ...
    function addFiles()
    {
		...
		if (is_dir($xoConfigs['download_sources'])) {
			if ($dh = opendir($xoConfigs['download_sources'])) {
				while (($file = readdir($dh)) !== false) {
					switch (filetype($xoConfigs['download_sources'] . '/' . $file)) {
					
					case 'file':
						$this->addOption($file, $file . sprintf(' (%d Mbs)',filesize($xoConfigs['download_sources'] . '/' . $file)/(1024*1024)));
						break;
					}
				}
				closedir($dh);
    ...
    function render()
    {
        ...
        $ele_value = array_unique($this->getValue());
