 ...
 * @access 		public 
 */
class VidshopFormSelectCategory extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function VidshopFormSelectCategory($caption, $name, $value = null, $size = 1, $multiple = false)
    {
    ...
    {
		$vidcatHandler =& xoops_getmodulehandler('video_category', 'vidshop');
		$criteria = new Criteria('cid', '0', '>');
		$criteria->setOrder('weight');
		$vidcat = $vidcatHandler->getObjects($criteria, true);
        if (is_array($vidcat)) {
            foreach($vidcat as $k => $v) {
                $this->addOption($v->getVar('cid'), $v->getVar('name'));
            }
        }
    }
    /**
