<?php
/*
  $Id: address_book_process.php 125 2006-01-21 16:15:58Z Michael $
...
*/
  ...
  $xoopsOption['template_main'] = 'address_book_process.html';
...
// reregister session variables
        if ( (isset($HTTP_POST_VARS['primary']) && ($HTTP_POST_VARS['primary'] == 'on')) || ($HTTP_GET_VARS['edit'] == $customer_default_address_id) ) {
		  ...
		  $_SESSION['customer_country_id'] = $coutry_id;
		  $_SESSION['customer_zon_id'] =$customer_zone_id;
...
//    $xoopsTpl->assign("nodelete",1);
	$xoopsTpl->assign("form_adr",tep_draw_form('addressbook', tep_href_link(FILENAME_ADDRESS_BOOK_PROCESS, (isset($HTTP_GET_VARS['edit']) ? 'edit=' . $HTTP_GET_VARS['edit'] : ''), 'SSL'), 'post', 'onSubmit="return check_form(addressbook);"'));
    $xoopsTpl->assign("form_end","</form>");
  }
  if(isset($HTTP_GET_VARS['edit'])) {
   $xoopsTpl->assign("title",HEADING_TITLE_MODIFY_ENTRY);
  }elseif(isset($HTTP_GET_VARS['delete'])) {
    $xoopsTpl->assign("title",HEADING_TITLE_DELETE_ENTRY);
	$xoopsTpl->assign("delete",1);
  }else{
    $xoopsTpl->assign("title",HEADING_TITLE_ADD_ENTRY);
  }
  $xoopsTpl->assign("site_img",tep_image(DIR_WS_IMAGES . 'table_background_address_book.gif', (isset($HTTP_GET_VARS['edit']) ? HEADING_TITLE_MODIFY_ENTRY : HEADING_TITLE_ADD_ENTRY), HEADING_IMAGE_WIDTH, HEADING_IMAGE_HEIGHT));
  ...
  $xoopsTpl->assign("address_book",tep_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL'));
  ...
  if (isset($HTTP_GET_VARS['delete'])) {
	...
	$xoopsTpl->assign("del_label",tep_address_label($customer_id, $HTTP_GET_VARS['delete'], true, ' ', '<br>'));

	$xoopsTpl->assign("address_book_del",tep_href_link(FILENAME_ADDRESS_BOOK_PROCESS, 'delete=' . $HTTP_GET_VARS['delete'] . '&action=deleteconfirm', 'SSL'));
	$xoopsTpl->assign("bt_del",tep_image_button('button_delete.gif', IMAGE_BUTTON_DELETE));
  } else {
    include(DIR_WS_MODULES . 'address_book_details.php');
    if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
		$xoopsTpl->assign("edit",1);
		$xoopsTpl->assign("edit_field",tep_draw_hidden_field('action', 'update'));
		$xoopsTpl->assign("edit_field1",tep_draw_hidden_field('edit', $HTTP_GET_VARS['edit']));
		$xoopsTpl->assign("bt_update",tep_image_submit('button_update.gif', IMAGE_BUTTON_UPDATE));
    } else {
	  ...
	  $xoopsTpl->assign("back_link",$back_link);
	  $xoopsTpl->assign("field_process",tep_draw_hidden_field('action', 'process'));
