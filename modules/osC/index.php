<?php
/**
 * $Id: index.php 91 2006-01-21 16:14:38Z Michael $
...
*/
	...
	$sql="select p.products_id, p.products_image,pd.products_name,p.products_tax_class_id, if(s.status, s.specials_new_products_price, p.products_price) as products_price from " . TABLE_PRODUCTS . " p left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id left join ".TABLE_PRODUCTS_DESCRIPTION." pd on pd.products_id=p.products_id  where p.products_status = '1' and pd.language_id=".$languages_id." and p.products_fsk18 <= ".$customer_group->getfsk18()." order by p.products_date_added desc limit 5";
	$new_products_result = $xoopsDB->queryF($sql);
	if($xoopsDB->getAffectedRows() > 0) {
    	$i=0;
		$xoopsTpl->assign("new_products",1);
		while($myrow =$xoopsDB->fetchArray($new_products_result)) {
			$new_products[$i] = $myrow;
	 		$new_products[$i]['products_price']=$currencies->display_price($new_products[$i]['products_price'], tep_get_tax_rate($new_products[$i]['products_tax_class_id']));
 	    	$i++;
		}
		$xoopsTpl->assign("new_products_list",$new_products);
	}
...
// We are asked to show only a specific category
        $listing_sql = "select " . $select_column_list . " p.products_id, p.manufacturers_id, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_MANUFACTURERS . " m, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id where p.products_status = '1' and p.manufacturers_id = m.manufacturers_id and m.manufacturers_id = '" . (int)$HTTP_GET_VARS['manufacturers_id'] . "' and p.products_id = p2c.products_id and pd.products_id = p2c.products_id and pd.language_id = '" . (int)$languages_id . "' and p2c.categories_id = '" . (int)$HTTP_GET_VARS['filter_id'] . "' and p.products_fsk18 <= ".$customer_group->getfsk18()." ";
      } else {
// We show them all
...
// We show them all
    ...
    if (PRODUCT_LIST_FILTER > 0) {
	  ...
	  $xoopsTpl->assign("filterlist",1);
	  $xoopsTpl->assign("filterform",tep_draw_form('filter', FILENAME_DEFAULT, 'get') . TEXT_SHOW. '&nbsp;');
        if (isset($HTTP_GET_VARS['manufacturers_id'])) {
          $xoopsTpl->assign("hiddenfield", tep_draw_hidden_field('manufacturers_id', $HTTP_GET_VARS['manufacturers_id']));
          $options = array(array('id' => '', 'text' => TEXT_ALL_CATEGORIES));
        } else {
          $xoopsTpl->assign("hiddenfield",  tep_draw_hidden_field('cPath', $cPath));
