<?php
/*
 $Id: packingslip_pdf.php 149 2006-01-27 13:31:07Z Michael $
...
$text_query = tep_db_query("SELECT * FROM ".$xoopsDB->prefix('osc_edit_invoice')." where edit_invoice_id = '3' and language_id = '" . $languages_id . "'");
...
for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
 ...
 $order->products[$i]['name'] = str_replace("</b>",'',$order->products[$i]['name']);
 $pdf->MultiCell(148,6,$order->products[$i]['name'].$attributes,1,'L');
 $pdf->SetY($Y_Table_Position);
 $pdf->SetX(178);
 $pdf->SetFont('Arial','',7);
