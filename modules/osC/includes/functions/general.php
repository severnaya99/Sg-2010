<?php
/**
 * $Id: general.php 65 2006-01-13 16:02:05Z Michael $
...
// TABLES: tax_rates, zones_to_geo_zones
  function tep_get_tax_rate($class_id, $country_id = -1, $zone_id = -1) {
    global $customer_zone_id, $customer_country_id,$xoopsDB;
...
// Add tax to a products price
  function tep_add_tax($price, $tax) {
    global $currencies,$customer_group;

    if ( ($customer_group->gettax() == '1') && ($tax > 0) ) {
      return tep_round($price, $currencies->currencies[DEFAULT_CURRENCY]['decimal_places']) + tep_calculate_tax($price, $tax);
    } else {
		//echo "add Tax".$price.":".$currencies->currencies[DEFAULT_CURRENCY]['decimal_places'].":".DEFAULT_CURRENCY."<br>";
...
//    echo "select entry_firstname as firstname, entry_lastname as lastname, entry_company as company, entry_street_address as street_address, entry_suburb as suburb, entry_city as city, entry_postcode as postcode, entry_state as state, entry_zone_id as zone_id, entry_country_id as country_id from " . TABLE_ADDRESS_BOOK . " where customers_id = '" . (int)$customers_id . "' and address_book_id = '" . (int)$address_id . "'"; 
...
// Return a customer greeting
  function tep_customer_greeting() {
    ...
    if (session_is_registered('customer_first_name') && session_is_registered('customer_id')) {
      $greeting_string = sprintf(TEXT_GREETING_PERSONAL, tep_output_string_protected($customer_first_name), tep_href_link(FILENAME_PRODUCTS_NEW));
    } else {
      $greeting_string = sprintf(TEXT_GREETING_GUEST, XOOPS_URL."/user.php", tep_href_link(FILENAME_CREATE_ACCOUNT, '', 'SSL'));
    }
...
// Return a random value
  ...
  function tep_count_customer_orders($id = '', $check_session = true) {
    ...
    if ($check_session == true) {
      if ( (session_is_registered('customer_id') == false) || ($id != $customer_id) ) {
        return 0;
      }
    }
